package com.walterlambardi.movement.handlandmarksframeprocessor

import android.util.Log
import com.walterlambardi.movement.HandLandmarkerHolder
...
class HandLandmarksFrameProcessorPlugin(proxy: VisionCameraProxy, options: Map<String, Any>?): FrameProcessorPlugin() {
  override fun callback(frame: Frame, arguments: Map<String, Any>?): Any? {
    ...
    try {
      ...
      return "Hand frame processed successfully"
    } catch (e: Exception) {
      e.printStackTrace()
      Log.e("HandLandmarksFrameProcessor", "Error processing hand frame: ${e.message}")
      return "Error processing hand frame: ${e.message}"
    }
  }
}